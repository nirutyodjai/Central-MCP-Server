...
async function runLoadTest() {
  ...
  try {
    ...
    Object.entries(results.performanceMetrics).forEach(([key, value]) => {
      console.log(
        `   ${key}: ${(value as any).averageTime?.toFixed(2) || 'N/A'}ms avg, ${(value as any).executions || 0} executions`
      );
    });
...
async function testCachePerformance(cache: FastCache, operations: number) {
...
async function testCodeSearchPerformance(
...
async function testBatchOperations(server: FastMCPServer, operations: number) {
...
async function stressTest(server: FastMCPServer, operations: number) {
  console.log(
